...
init( Req, State ) ->
    ...
    WebSocket = cowboy_req:header(<<"upgrade">>,Req),
      ...
      Value ->
        ...
        case chat_room_server:get_connected_user(CryptedUserName) of
          ...
          _ ->
            ...
            %% lets chack again.
            case chat_room_server:connect_user({Value, CryptedUserName}) of%% THINK again!! 
              already_exists ->
                enter_home_page(already_exists, Body, Req, State);
            ok ->
...
enter_home_page(Flag, Body, Req, State) ->
    {HTTPResponse, Title} =
      case Flag of
        enter ->
          {200, "Welcome. Please enter your name"};
        already_exists ->
          {401, "Please enter under another user since he has already connected."};
        does_not_exist ->
          {401, "Please enter user name again since you have tried to send a message under not connected user."}
      end,
